...
async def handle_vivian_functions_enhanced(run, thread_id):
    ...
    for tool_call in run.required_action.submit_tool_outputs.tool_calls:
        ...
        try:
            ...
            elif function_name == "read_briefing_notes":
                output = read_briefing_notes()
            
            elif function_name == "generate_work_briefing":
                briefing_type = arguments.get('type', 'morning')
                # For OpenAI Assistant, return text format not embeds
                briefing_notes = read_briefing_notes()
                calendar_summary = get_work_calendar_summary()
                output = f"**Work Briefing ({briefing_type.title()})**\n\n{briefing_notes}\n\n---\n\n{calendar_summary}"
            
            elif function_name == "generate_work_review":
                # For OpenAI Assistant, return text format not embeds
                briefing_notes = read_briefing_notes()
                calendar_summary = get_work_calendar_summary()
                output = f"**End-of-Day Work Review**\n\n{briefing_notes}\n\n---\n\n{calendar_summary}"
            
            elif function_name == "get_work_calendar_summary":
                output = get_work_calendar_summary()
...
# ============================================================================
# ENHANCED WORK BRIEFING FUNCTIONS
# ============================================================================
...
def get_work_calendar_summary():
    ...
    try:
        ...
        today_events = get_work_schedule_today()
        
        # Get upcoming work events (next 3 days)
        upcoming_events = get_work_schedule_upcoming(3)
        
        return f"""📅 **Calendar Integration**
...
{upcoming_events}"""
        ...
        return f"📅 **Calendar:** Error retrieving work calendar - {str(e)}"

def parse_briefing_sections(briefing_notes):
    """Parse briefing notes into organized sections for Discord embeds"""
    sections = {}
    current_section = None
    current_content = []
    
    lines = briefing_notes.split('\n')
...
def generate_work_briefing_embeds(briefing_type="morning"):
    """Generate work briefing as Discord embeds with proper formatting"""
    toronto_tz = pytz.timezone('America/Toronto')
    current_time = datetime.now(toronto_tz)
    
    # Read and parse briefing notes
    briefing_notes = read_briefing_notes()
    sections = parse_briefing_sections(briefing_notes)
    
    # Get calendar data
    today_events = get_work_schedule_today()
    
    if briefing_type == "morning":
        title = "🌅 Comprehensive Work Briefing"
        description = f"Morning Strategy Session - {current_time.strftime('%A, %B %d, %Y')}"
        color = 0x0F4C75
    else:
        title = "🌆 End-of-Day Work Review" 
        description = f"Daily Wrap-Up - {current_time.strftime('%A, %B %d, %Y')}"
        color = 0x2E4A66
    ...
    # Add strategic context (priority section)
    if '🎯 Strategic Context' in sections:
        context = sections['🎯 Strategic Context']
        if len(context) > 1024:
            context = context[:1020] + "..."
    ...
    if today_events and len(today_events) < 800:
        main_embed.add_field(
    ...
    # Project Portfolio embed
    if '📊 Project Portfolio Metrics' in sections or '📊 IN PROGRESS PROJECTS' in sections:
        project_embed = discord.Embed(
            title="📊 Project Portfolio Status",
            color=color
        )
        
        if '📊 Project Portfolio Metrics' in sections:
            metrics = sections['📊 Project Portfolio Metrics'][:1024]
        ...
        if '📊 IN PROGRESS PROJECTS' in sections:
            in_progress = sections['📊 IN PROGRESS PROJECTS']
            # Split if too long
            if len(in_progress) > 1024:
                project_embed.add_field(
                    name="🔄 Active Projects",
                    value=in_progress[:1020] + "...",
                    inline=False
                )
            else:
                project_embed.add_field(
    ...
    # Blockers and Communications embed
    comms_embed = discord.Embed(
        title="💬 Communications & Blockers",
        color=color
    )
    
    if '⏸️ BLOCKED/WAITING PROJECTS' in sections:
        blocked = sections['⏸️ BLOCKED/WAITING PROJECTS'][:1024]
        comms_embed.add_field(
            name="⏸️ Blocked Projects",
            value=blocked,
            inline=False
        )
    
    if '💬 Communications Priorities' in sections:
        comms = sections['💬 Communications Priorities'][:1024] 
        comms_embed.add_field(
            name="💬 Priority Communications",
            value=comms,
            inline=False
        )
    
    # Only add if it has content
    if len(comms_embed.fields) > 0:
        embeds.append(comms_embed)
    
    # Add footer to last embed
    if embeds:
        embeds[-1].set_footer(text="💼 Generated by Vivian Spencer | Strategic Work Intelligence")
    
    return embeds

def generate_work_review():
    """Generate end-of-day work review briefing"""
    return generate_work_briefing_embeds(briefing_type="review")
...
async def workbriefing_command(ctx):
    ...
    try:
        ...
        # Send each embed with a small delay for better presentation
        for i, embed in enumerate(briefing_embeds):
            await ctx.send(embed=embed)
            if i < len(briefing_embeds) - 1:  # Don't delay after the last embed
                await asyncio.sleep(0.5)
        
        print("✅ Work briefing sent successfully")
...
async def workreview_command(ctx):
    ...
    try:
        ...
        # Send each embed with a small delay for better presentation
        for i, embed in enumerate(review_embeds):
            await ctx.send(embed=embed)
            if i < len(review_embeds) - 1:  # Don't delay after the last embed
                await asyncio.sleep(0.5)
        
        print("✅ Work review sent successfully")
        
    except Exception as e:
        print(f"❌ Work review command error: {e}")
        import traceback
        traceback.print_exc()
        await ctx.send("💼 Work review unavailable. Please try again.")
...
async def send_automated_work_briefing():
    ...
    try:
        ...
        if target_channel:
            ...
            print("✅ Automated work briefing sent successfully")
            
        else:
            print(f"❌ Target channel not found for automated work briefing")
...
async def send_automated_work_review():
    ...
    try:
        ...
        if target_channel:
            ...
            print("✅ Automated work review sent successfully")
            
        else:
            print(f"❌ Target channel not found for automated work review")
