    ...
    for tool_call in run.required_action.submit_tool_outputs.tool_calls:
        ...
        try:
            ...
            if function_name == "get_work_schedule_today":
                ...
                output = generate_work_briefing(briefing_type)
            
            elif function_name == "generate_work_review":
                output = generate_work_review()
...
def generate_work_briefing(briefing_type="morning"):
    """Generate comprehensive work briefing combining notes and calendar"""
    toronto_tz = pytz.timezone('America/Toronto')
    current_time = datetime.now(toronto_tz)
    
    # Read the briefing notes content
    ...
    if briefing_type == "morning":
        ...
        time_context = f"Morning Strategy Session - {current_time.strftime('%A, %B %d, %Y at %-I:%M %p')}"
    else:  # review
        title = "🌅 **End-of-Day Work Review**"
        time_context = f"Daily Wrap-Up - {current_time.strftime('%A, %B %d, %Y at %-I:%M %p')}"
...
💼 **Generated by Vivian Spencer** | Strategic Work Intelligence"""

    return briefing

def generate_work_review():
    """Generate end-of-day work review briefing"""
    return generate_work_briefing(briefing_type="review")
...
async def workbriefing_command(ctx):
    ...
    try:
        ...
        briefing_content = generate_work_briefing(briefing_type="morning")
        
        # Send as embed for better formatting
        embed = discord.Embed(
            title="🌅 Comprehensive Work Briefing",
            description="Strategic project status with calendar integration",
            color=0x0F4C75,
            timestamp=datetime.now(timezone.utc)
        )
        
        # Split content if it's too long for Discord
        if len(briefing_content) > 4000:
            # Send first part as embed
            embed.add_field(
                name="📊 Strategic Overview",
                value=briefing_content[:4000] + "...",
                inline=False
            )
            await ctx.send(embed=embed)
            
            # Send remaining content as follow-up
            remaining = briefing_content[4000:]
...
async def workreview_command(ctx):
    ...
    try:
        ...
        # Generate the review briefing
        review_content = generate_work_review()
        
        # Send as embed for better formatting
        embed = discord.Embed(
            title="🌆 End-of-Day Work Review",
            description="Daily wrap-up and tomorrow's preparation",
            color=0x0F4C75,
            timestamp=datetime.now(timezone.utc)
        )
        ...
        if len(review_content) > 4000:
            ...
            embed.add_field(
                name="📊 Daily Review",
                value=review_content[:4000] + "...",
                inline=False
            )
            await ctx.send(embed=embed)
            
            # Send remaining content as follow-up
            remaining = review_content[4000:]
            chunks = [remaining[i:i+2000] for i in range(0, len(remaining), 2000)]
            for chunk in chunks:
                await ctx.send(f"```\n{chunk}\n```")
        else:
...
async def send_automated_work_briefing():
    ...
    try:
        ...
        if target_channel:
            ...
            embed = discord.Embed(
                title="🌅 Automated Work Briefing",
                description="9 AM Strategic Project Status",
                color=0x0F4C75,
                timestamp=datetime.now(timezone.utc)
            )
...
async def send_automated_work_review():
    ...
    try:
        ...
        if target_channel:
            ...
            # Send with formatting
            embed = discord.Embed(
                title="🌆 Automated Work Review",
                description="4:45 PM End-of-Day Wrap-Up",
                color=0x0F4C75,
                timestamp=datetime.now(timezone.utc)
            )
